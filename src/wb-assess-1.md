...
```
console.log is a function used to see what the given inputs would output but is only accessable in the console. return is a keyword that java script uses to end functions once the function returns the return statement.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function sets the conditions and the contents of a function. Calling a function tells the prorgam to exicute the code funciton.
```
...
```
A string is a data type that can be made of numbers, symbols and characters. Strings are concatinated.
```

**_Number_**

```
A number is a data type that represents whole numbers and decimals. Numbers are used in mathematical operations.
```

**_Boolean_**

```
A boolean is a logical operator that takes statements and expressions and applies logic to them to return a result based on the logic.
```

**_Array_**

```
An array is a data type that stores multiple values into one varialble.
```
